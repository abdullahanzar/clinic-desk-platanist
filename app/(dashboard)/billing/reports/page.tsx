  ...
  if (!report) {
    return (
      <div className="text-center py-12">
        <p className="text-slate-500 dark:text-slate-400">Failed to load report</p>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mb-6 sm:mb-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">Financial Reports</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Generate and export billing reports</p>
          </div>
          <button
            onClick={exportToCSV}
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 sm:p-6 mb-6 shadow-sm">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl">
            <p className="text-xs text-emerald-600 dark:text-emerald-400 uppercase tracking-wide">Collected</p>
            <p className="text-2xl font-bold text-emerald-700 dark:text-emerald-300">₹{report.revenue.collected.toLocaleString()}</p>
            <p className="text-xs text-emerald-600 dark:text-emerald-400">{report.revenue.paidCount} receipts</p>
          </div>
          <div className="p-4 bg-amber-50 dark:bg-amber-900/30 rounded-xl">
            ...
            <p className="text-2xl font-bold text-amber-700 dark:text-amber-300">₹{report.revenue.pending.toLocaleString()}</p>
            <p className="text-xs text-amber-600 dark:text-amber-400">{report.revenue.unpaidCount} receipts</p>
          </div>
          <div className="p-4 bg-violet-50 dark:bg-violet-900/30 rounded-xl">
            <p className="text-xs text-violet-600 dark:text-violet-400 uppercase tracking-wide">Discounts Given</p>
            <p className="text-2xl font-bold text-violet-700 dark:text-violet-300">₹{report.revenue.totalDiscount.toLocaleString()}</p>
          </div>
          <div className="p-4 bg-slate-50 dark:bg-slate-700 rounded-xl">
            <p className="text-xs text-slate-600 dark:text-slate-400 uppercase tracking-wide">Avg Receipt</p>
            <p className="text-2xl font-bold text-slate-700 dark:text-slate-200">₹{report.revenue.averageReceiptValue.toLocaleString()}</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 sm:p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-700">
              <span className="text-slate-600 dark:text-slate-400">Revenue (Collected)</span>
              <span className="font-semibold text-emerald-600 dark:text-emerald-400">+₹{report.profitLoss.revenue.toLocaleString()}</span>
            </div>
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-700">
              <span className="text-slate-600 dark:text-slate-400">Expenses</span>
              <span className="font-semibold text-red-600 dark:text-red-400">-₹{report.profitLoss.expenses.toLocaleString()}</span>
            </div>
            <div className="flex items-center justify-between pt-2">
              <span className="font-semibold text-slate-900 dark:text-slate-100">Net Profit</span>
              <div className="text-right">
                <span className={`text-xl font-bold ${report.profitLoss.netProfit >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400"}`}>
                  {report.profitLoss.netProfit >= 0 ? "+" : ""}₹{report.profitLoss.netProfit.toLocaleString()}
                </span>
                <p className="text-xs text-slate-500 dark:text-slate-400">{report.profitLoss.profitMargin}% margin</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-2">
                  <span className="text-slate-600 dark:text-slate-400">Target: ₹{report.budget.targetRevenue.toLocaleString()}</span>
                  <span className={`font-medium ${report.budget.revenueAchieved >= 100 ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}>
              ...
              <div className="pt-4 border-t border-slate-100 dark:border-slate-700">
                <div className="flex justify-between">
                  <span className="text-slate-600 dark:text-slate-400">Actual Revenue</span>
                  <span className="font-semibold text-slate-900 dark:text-slate-100">₹{report.profitLoss.revenue.toLocaleString()}</span>
                </div>
                <div className="flex justify-between mt-2">
                  <span className="text-slate-600 dark:text-slate-400">Gap to Target</span>
                  <span className={`font-semibold ${report.budget.revenueGap > 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}>
          ...
          {report.paymentModes.length > 0 ? (
            <div className="space-y-3">
              {report.paymentModes.map((mode) => (
                <div key={mode.mode} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700 rounded-xl">
                  <div className="flex items-center gap-3">
                    <span className="font-medium text-slate-700 dark:text-slate-200 capitalize">
                      {mode.mode === "upi" ? "UPI" : mode.mode}
                    </span>
                    <span className="text-xs text-slate-500 dark:text-slate-400">({mode.count})</span>
                  </div>
                  <span className="font-semibold text-slate-900 dark:text-slate-100">₹{mode.amount.toLocaleString()}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-slate-500 dark:text-slate-400 text-center py-8">No payment data</p>
          )}
        </div>
            ...
            <div className="space-y-3">
              {report.topServices.slice(0, 5).map((service, index) => (
                <div key={service.service} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700 rounded-xl">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-slate-900 dark:text-slate-100">₹{service.amount.toLocaleString()}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{service.count}x</p>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-slate-500 dark:text-slate-400 text-center py-8">No service data</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-5 sm:p-6 shadow-sm">
        ...
        {report.expenses.byCategory.length > 0 ? (
          ...
          <p className="text-slate-500 dark:text-slate-400 text-center py-8">No expenses recorded for this period</p>
        )}
      </div>
    </div>
  );
