...
export default function AdviceTab() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch("/api/templates/advice/seed");
      if (res.ok) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setDeleteError("Failed to delete advice");
      setTimeout(() => setDeleteError(null), 3000);
    }
      ...
      {stats && (
        <div className="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h4 className="text-sm font-medium text-slate-900 mb-2">Advice Database</h4>
      ...
      {totalCount > ITEMS_PER_PAGE && (
        <div className="mt-6 flex items-center justify-between">
          <p className="text-sm text-slate-500">
            Showing {((currentPage - 1) * ITEMS_PER_PAGE) + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, totalCount)} of {totalCount} advice templates
          </p>
